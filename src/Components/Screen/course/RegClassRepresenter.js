...
class RegClassRepresenter extends Component {
  ...
  render() {
    ...
    const registra = ["DEAN"]
    const data_role = roles && roles.filter((obj) => !registra.includes(obj.name));
    const custom_role = isLoggedIn === "DEAN" && data_role ? data_role.filter(obj => deanArr.includes(obj.name))
                        : isLoggedIn === "TEACHER" ? data_role.filter(obj => teacherArr.includes(obj.name))
                        : data_role
